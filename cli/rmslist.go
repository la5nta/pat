...
func RMSListHandle(ctx context.Context, a *app.App, args []string) {
	...
	byLinkQuality := set.BoolP("sort-link-quality", "q", false, "Sort by predicted link quality (implies -p)")
	predict := set.BoolP("predict", "p", false, "Predict propagation and show reliability score")
	set.Parse(args)
	...
	*mode = strings.ToLower(*mode)
	rList, err := a.ReadRMSList(ctx, *forceDownload, *predict, func(rms app.RMS) bool {
		switch {
	...
	for i, r := range rList {
		qual := "N/A"
		if r.LinkQuality > -1 {
			qual = fmt.Sprintf("%d%%", r.LinkQuality)
		}
		printRMS(r, qual)
