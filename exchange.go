...
func (m NotifyMBox) GetInboundAnswers(p []fbb.Proposal) []fbb.ProposalAnswer {
	...
	// Prompt the user
	ans := <-promptHub.Prompt(context.Background(), PromptKindMultiSelect, "Select messages for download", options...)
...
func sessionExchange(conn net.Conn, targetCall string, master bool) error {
	...
	session.SetSecureLoginHandleFunc(func(addr fbb.Address) (string, error) {
		...
		resp := <-promptHub.Prompt(context.Background(), PromptKindPassword, "Enter secure login password for "+addr.String())
