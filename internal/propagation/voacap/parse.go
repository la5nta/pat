...
scanLoop:
	for scanner.Scan() {
		...
		case strings.HasSuffix(strings.TrimSpace(line), "FREQ"):
			...
			if len(fields) < 3 { // e.g., 0.0 UT 2.0 FREQ
			...
			freqs := fields[1 : len(fields)-1]
			for _, freqStr := range freqs {
		...
		case currentPrediction != nil && strings.TrimSpace(line) != "" && !strings.Contains(line, "*****END OF RUN*****"):
			paramFields := strings.Fields(line)
			...
			// Check if the param name is two words by trying to parse the second to last field as a float.
			if _, err := parseFloat(paramFields[len(paramFields)-2]); err != nil {
				// It's not a float, so it's part of the name.
				paramName = strings.Join(paramFields[len(paramFields)-2:], " ")
				paramValues = paramFields[0 : len(paramFields)-2]
			} else {
				paramName = paramFields[len(paramFields)-1]
				paramValues = paramFields[0 : len(paramFields)-1]
			}
